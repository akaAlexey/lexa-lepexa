...
export const tokens = {
  ...
  touchMin: '3rem',
  touchMain: '3.5rem',
  /** Карта-хаб: шторка на телефоне (px). */
  map: {
    /** Зазор между полем поиска и поднятой до конца шторкой. */
    sheetGap: 8,
    /** Место под карту над видимой частью шторки: метки не прячутся под её краем. */
    sheetClearance: 16,
  },
} as const
