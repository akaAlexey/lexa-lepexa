...
function Locked({ onSignIn }: { onSignIn: () => void }) {
  return (
    <>
      <Notice>
        Раздел откроется после входа: в нём будут личные данные вашей семьи, поэтому он не
        виден без профиля.
      </Notice>
    ...
    <form className={s.form} onSubmit={submit} noValidate data-testid="signin-form">
      ...
      <p className={s.links}>
        ...
        <button
          type="button"
          className={s.linkButton}
          onClick={() => setNote('Регистрация появится вместе с сервером входа. Сейчас можно войти с любым телефоном или почтой.')}
        >
          Зарегистрироваться
        </button>
      </p>
