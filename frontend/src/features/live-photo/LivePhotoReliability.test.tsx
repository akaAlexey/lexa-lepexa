...
/** Обычная камера (AR-режим «Другого») в тестах «Живого фото» не нужна. */
const noCamera = () => Promise.reject(new Error('Камеры нет в тестовой среде'))
...
describe('«Живое фото»: камера', () => {
  ...
  it.each([
    ...
    renderApp('/live/soldier', {
      signedIn: true,
      platform: {
        ar: { openCamera: noCamera, trackImage: () => Promise.reject(new DOMException('x', name)) },
      },
    })
    await openCamera()
