...
> **Статус: тесты приёмки написаны (этап 3), реализация — этап 4.** Работают навигация по ролям, тема, API-слой (mock/live), карта. Поведение трёх user story описано тестами: [docs/ACCEPTANCE.md](docs/ACCEPTANCE.md); пока они красные — `npm run verify` станет зелёным к концу этапа 4. Итоговая документация по всем осям появится к упаковке (требования — в [docs/HANDOFF.md](docs/HANDOFF.md)).
...
Все данные сейчас демонстрационные и помечены в интерфейсе. Файлы в формате жюри (`fixtures/jury/`) сгенерированы по схеме кейса и вымышлены. Источники, на которые ссылается интерфейс: «Книга Памяти» Орловской области, ОБД «Мемориал» Минобороны России, «Память народа», OpenStreetMap.
